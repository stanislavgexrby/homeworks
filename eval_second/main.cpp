...
private:
    ...
    int m = 0; // m+1 - число значений в таблице
    ...
    int getColumnWidth(const string& header, bool isNumeric = true) {
        if (isNumeric) {
            // Для чисел: учитываем знак, цифры до запятой, точку, precision и запас
...
public:
    ...
    void createTable() {
        ...
        cout << "\nВведите число значений в таблице (m+1): ";
        int table_size;
        cin >> table_size;
        m = table_size - 1;
        ...
        for (int k = 0; k <= m; k++) {
            ...
            entry.z = z;
            entry.fz = f(z);
            entry.dist_to_x = 0.0; // заполним позже
            table.push_back(entry);
        }
        
        cout << "\nИсходная таблица значений функции:" << endl;
        
        int w1 = 4;  // ширина для k
    ...
    double lagrangeInterpolation(double x, int n) {
        ...
        double result = 0.0;
        double sum_coefficients = 0.0; // Контроль: сумма коэффициентов должна быть 1
